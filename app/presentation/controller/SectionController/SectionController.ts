...
export default class SectionController {
    ...
    observableSectionFinish = new Obeservable<SectionModel>();

    async getSectionByUid(uid: string, withLoad: boolean) {
        if (uid) {
            if (withLoad) {
                this.observableSectionGet.emit(
                    new LoadingUiState<SectionModel[]>()
                );
            }
            await this.sectionUseCase
                .getSectionByUid(uid)
                .then((response) => {
                    if (response instanceof SuccessResource) {
                        this.observableSectionGet.emit(
                            new SuccessUiState<SectionModel[]>(response.data)
                        );
                    } else if (response instanceof ErrorResource) {
                        this.observableSectionGet.emit(
                            new ErrorUiState<SectionModel[]>(
                                response.toUiError()
                            )
                        );
                    } else {
    ...
    async registerNewAddressAnSection(placeModel: PlaceModel, uid: string) {
        this.observableSectionNewWithPlace.emit(
            new LoadingUiState<SectionModel>()
        );
        await this.sectionUseCase
            .createSectionWithPlace(placeModel, uid)
            .then((value) => {
                if (value instanceof SuccessResource) {
                    this.observableSectionNewWithPlace.emit(
                        new SuccessUiState(value.data)
                    );
    ...
    async finishSection(section: SectionModel) {
        this.observableSectionFinish.emit(new LoadingUiState<SectionModel>())

        this.sectionUseCase.finishSection(section)
        .then((section) => {
            if (section instanceof SuccessResource) {
                this.observableSectionFinish.emit(
                    new SuccessUiState(section.data)
                );
            } else if (section instanceof ErrorResource) {
                console.error(section.error);
                this.observableSectionFinish.emit(
                    new ErrorUiState<SectionModel>({
                        code: section.error?.code,
                        mensage: section.error?.mensage,
                    })
                );
            }
        })
